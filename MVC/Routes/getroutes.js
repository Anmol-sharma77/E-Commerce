...
function getproducts(request,response){
    ...
    functionroutes.getproducts(refresh,function(error,data){
     if(error){
      response.status(401);
      response.send();
     } 
...
function getdash(request,response){
    response.render("dashboard",{error:null});
  }
...
async  function rejectrequest(request,response)
  ...
  async function cancelorders(request,response)
  {
    id=request.session.userid;
    try{
      var orders=await queryAsync(`select * from orders where ordstatus="canceled" and sellerid=${id}`);
      let data2=[];
      for(i=0;i<orders.length;i++){
        var product= await queryAsync(`select product,price,filename from products where prid=${orders[i].prid}`);
        orders[i].product=product[0].product;
  ...
  async function deliversorders(request,response)
  {
    id=request.session.userid;
    try{
      var orders=await queryAsync(`select * from orders cross JOIN products on products.prid=orders.prid where orders.ordstatus="delivered" and orders.sellerid=${id};`);
      response.status(200);
